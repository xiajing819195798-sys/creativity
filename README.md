...
- `ai`：学生先提出两个想法，再与 CloudBase AI 对话；每次 AI 回复后，学生需选择“接受、部分接受或不接受”并说明理由，最后形成最终方案。

同一学生在同一版本中刷新或恢复作答时不会改变组别。提交数据包含分组、初步想法、独立反思或 AI 对话记录、学生对每条 AI 建议的判断与理由、最终方案，以及最终方案的修改事件和分段版本记录。

## 作答努力程度筛查

完成测评时，网页会运行保守的自动筛查。筛查综合空白题、明显无效或占位文本、各题最低作答量、最后一题关键步骤完整性和异常短的总作答时间；不会仅因答案缺乏创意或只出现一个轻微信号就判定为不努力。达到组合阈值时，页面提示后续进行二轮作答。

该结果属于筛查标记，不是创造力分数或最终研究结论。数据库 `payload` 会保存 `needs_second_round`、筛查版本、信号数量、原因代码和完整检查 JSON，便于研究者复核。阈值版本为 `effort_screen_v1`，正式施测前应通过认知访谈和预测试校准误判率。
...
每次提交的 `payload` 字段包含五道题全部答案、每题用时、随机分组、完整 AI 对话（学生和 AI 的每条消息、时间和模型）、独立反思内容、AI 建议判断、最终答案修改轨迹以及完整答案 JSON。最终答案轨迹同时保留逐次输入事件的时间、操作类型和文本长度，以及在输入停顿、失焦或提交时形成的完整文本版本。为了方便直接导出为一题一列的数据表，可在 CloudBase SQL 编辑器运行 `db_export_view.sql`，之后导出视图 `public.creativity_submissions_export`。
